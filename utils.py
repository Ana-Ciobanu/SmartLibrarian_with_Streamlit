...
def chatgpt_response(user_input, book_title, client, TEXT_MODEL):
    ...
    prompt = f"""
        ...
        Use the get_summary_by_title tool to get the complete summary which you will display as it is.
        """
    input_list = [
    ...
    try:
        ...
        if function_call and function_call_arguments:
            ...
            response = client.responses.create(
                model=TEXT_MODEL,
                instructions="Respond only with a book recommendation generated by the tool. If no book found, write the message 'Unfortunately, I wasn’t able to find a matching book in the library. Would you like me to try again with a different topic or interest?'",
                tools=tools,
                input=input_list,
            )
